...
export class FlyAgent {
  constructor({ mj, flyXML, env, data, size, sign, bodymap, gait, id = 0, pos = [0, 0], yaw = 0, nProxies = 0, mode = 'descending', brainOpts = {}, vision = true, brain = null }) {
    ...
    this.geomKind = []; for (let g = 0; g < M.ngeom; g++) { const n = M.geom(g).name; this.geomKind.push(n === 'floor' ? 'floor' : n.startsWith('wall') ? 'wall' : n.startsWith('food') ? 'food' : n.startsWith('bitter') ? 'bitter' : n.startsWith('hazard') ? 'hazard' : n.startsWith('obst') ? 'obst' : n.startsWith('proxy') ? 'fly' : 'self'); }
    ...
    this.eye = vision ? new CompoundEye(mj, M, this.mjd, bodymap, this.bid.head, this.bid.thorax) : null;
    this.motor = new Motor(mj, M, this.mjd, bodymap, typeOf, sideOf, gait, mode);
    // lamina monopolar cells: graded resting depolarisation so light (histaminergic inhibition) can modulate them
    this.lamina = []; for (let i = 0; i < data.N; i++) if (/^L[1-5]$/.test(typeOf[i])) this.lamina.push(i);
    this.brain.setBias(this.lamina, brainOpts.laminaBias ?? 9);
  ...
  step() {
    ...
    for (let s = 0; s < 10; s++) mj.mj_step(M, d);
    this.t += 1;
    this.physiology(st);
  }
  physiology(st) {
