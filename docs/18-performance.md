...
| Physics | Model and data freed when an agent is done with them (`FlyAgent.dispose`) | About 28 MB an agent inside the emscripten heap, which the collector never reclaims; the headless tools build one per scenario and reached the 2 GB heap limit after 76 of them |
