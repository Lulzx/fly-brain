...
const OUT = { fwd: [...T('DNg100'), ...T('DNg97'), ...T('DNp09')], MDN: T('MDN'), steerL: [...T('DNa02', 1), ...T('DNa01', 1)], steerR: [...T('DNa02', 2), ...T('DNa01', 2)], GF: T('DNp01'), MN9: T('MN9') };
...
for (const [name, ix] of channels) {
  ...
  if (name.startsWith('eye')) { const all = D.bodymap.eyes.flatMap(e => e.idx); b.setDrive(all, 40); b.setDrive(ix, 0); }  // a dark region = photoreceptors fall silent
  else b.setDrive(ix, 100);
...
for (const k of Object.keys(OUT)) show(k);
