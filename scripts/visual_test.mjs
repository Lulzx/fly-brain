...
const mem = allocBrainMemory(data, size, sign, params, 1); const brain = await attachBrain(fs.readFileSync('public/lif.wasm'), mem, 0, data, 9);
const fly = new FlyAgent({ mj, flyXML, env, data, size, sign, bodymap: D.bodymap, gait, brain, brainOpts: params, nProxies: 1, vision: true });
const T = (t, s) => D.byType(t).filter(i => !s || D.side[i] === s);
const G = { fwd: [...T('DNg100'), ...T('DNg97'), ...T('DNp09')], P9L: T('DNp09', 1), P9R: T('DNp09', 2), DNaL: [...T('DNa02', 1), ...T('DNa01', 1)], DNaR: [...T('DNa02', 2), ...T('DNa01', 2)], GF: T('DNp01'), MDN: T('MDN') };
const vpn = []; for (let i = 0; i < D.N; i++) if (D.meta.superclasses[D.sc[i]] === 'visual_projection') vpn.push(i);
const stim = process.argv[2] || 'circle';
const M = fly.model, d = fly.mjd, mid = M.body_mocapid[M.body('proxy0').id];
...
for (let t = 0; t < 3000; t++) {
  ...
  if (stim === 'loom') { const r = Math.max(0.12, 2.0 - t / 1000 * 1.2); x = r; y = 0; z = 0.13; }                            // approaching head-on
  ...
  if (t % 10 === 0) { fly._eyeRates = new Map(); const er = fly._eyeRates; fly.eye.update({ set: (ix, hz) => { for (const i of ix) er.set(i, hz); } }, env, 10, fly.albedo); }
  ...
  if ((t + 1) % 250 === 0) {
    const r = Object.fromEntries(Object.entries(G).map(([k, v]) => [k, v.reduce((a, i) => a + brain.spikeCount[i] - prev[i], 0) / v.length / 0.25]));
    let va = 0; for (const i of vpn) if (brain.spikeCount[i] !== prev[i]) va++;
    const az = Math.atan2(y, x) * 180 / Math.PI;
    console.log(`t=${((t + 1) / 1000).toFixed(2)} obj az ${az.toFixed(0)}° | VPNs active ${va} | ` + Object.entries(r).map(([k, v]) => `${k} ${v.toFixed(0)}`).join(' '));
